...
class newcomers(commands.Cog):
    ...
    async def on_member_join(self, user: discord.Member):
        ...
        susalt = (90 - days)
        altban = str(susalt * 24 * 60 * 60)

        if days == 0:
            await self.tempban(user, altban)
            await user.ban(reason="Suspected ALT, Banned for 90 days.")
            channel = self.bot.get_channel(676931619294281729)
            await channel.send(f"Auto Banned {user} `{user.id}` for being a suspected ALT, Come back in {susalt} days")

        elif days < 14:
            await self.tempban(user, bannedday)
            await user.ban(
            ...
            for x in fetchall:
                if current_time >= x["BannedUntil"]:  # do stuff after this
                    ...
                    try:
                        ...
                        await channel.send(f"Hey <@705769248034914314>, I failed to unban {unbanuser} can you have a look?")
